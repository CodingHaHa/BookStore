...
function validateLoginpass() {
	...
	if(!value) {// 非空校验
		...
		$("#loginpassError").text("不能为空");
		bool = false;
	} else if(value.length < 3 || value.length > 20) {//长度校验
		$("#loginpassError").css("display", "");
		$("#loginpassError").text("长度3 ~ 20之间");
		bool = false;
	} ;
	return bool;
}
...
function validateNewloginpass() {
	...
	if(!value) {// 非空校验
		...
		$("#newpassError").text("不能为空");
		bool = false;
	} else if(value.length < 3 || value.length > 20) {//长度校验
		$("#newpassError").css("display", "");
		$("#newpassError").text("长度3 ~ 20之间");
		bool = false;
	}
	return bool;
}
...
function validateReloginpass() {
	...
	if(!value) {// 非空校验
		...
		$("#reloginpassError").text("不能为空");
		bool = false;
	} else if(value != $("#newpass").val()) {//两次输入是否一致
		$("#reloginpassError").css("display", "");
		$("#reloginpassError").text("输入不一致");
		bool = false;
	}
	return bool;	
}
...
function validateVerifyCode() {
	...
	if(!value) {//非空校验
		...
		$.ajax({
			...
			type: "POST",
			dataType: "json",
			data: {method: "ajaxVaildateVerifyCode", verifyCode: value},
